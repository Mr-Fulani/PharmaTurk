...
- **Привязка к товарам:** при обработке товара берётся **категория товара** (`product.category`). Система ищет шаблон с таким же типом по **дереву категорий**: сначала точная категория товара, затем её **родители вверх до корня**, затем шаблон **без категории** (общий), иначе встроенный дефолт. Поэтому шаблон на корневой категории (напр. «Парфюмерия») автоматически действует на все подкатегории («Духи», «Мужские ароматы» и т.д.) — заводить шаблон на каждую подкатегорию не нужно.
- **Сид по умолчанию:** базовые `category_instruction` заводятся командой `python manage.py seed_ai_category_instructions` (идемпотентно, текст версионируется в git, дальше правится в админке). Сейчас сидится парфюмерия.
...
| **Анализ изображения** (`image_prompt`) | Промпт для Vision API (что спрашивать по картинке). | При анализе фото: точная категория → её родители до корня → общий шаблон → встроенный дефолт. |
...
**Что будет:** инструкция применяется к товару той же категории и ко всем её
подкатегориям, пока более близкая категория в цепочке не задаёт собственный
шаблон. Товары вне этой ветки дерева инструкцию не получат.
...
**Что будет:** при анализе фото используется ближайший шаблон в цепочке
категорий. Для товара без подходящего exact/parent шаблона применяется общий
`image_prompt` без категории, затем встроенный дефолт.

---

## Важно про категорию товара

- Шаблон срабатывает, если категория товара совпадает с категорией шаблона или
  является её потомком; выбирается ближайшее совпадение вверх по дереву.
...
| К каким товарам применяется? | К товарам этой категории и её подкатегорий, если ниже по дереву нет более точного шаблона. |
| Какие типы учитывают категорию? | **Инструкция для категории** (текст в промпт) и **Анализ изображения** (промпт для Vision). |
| Общие шаблоны без категории? | Оставьте **«Категория»** пустым — такой шаблон используется, когда нет подходящего exact/parent шаблона. |

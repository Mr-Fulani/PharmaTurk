...
- **LLM (OpenAI/Local)**: Генерация текста и анализ изображений.
...
3. **AI Worker**:
   ...
   - Анализирует изображения (Vision API).
   - Определяет категорию (RAG + LLM).
   - Генерирует описание и SEO (LLM).
4. **Результат** сохраняется в `AIProcessingLog`.
5. **Применение**: Если уверенность высокая — применяется автоматически. Если низкая — отправляется на модерацию.
...
- **`AIProcessingLog`**: Основной журнал обработки. Хранит:
  - Входные данные (JSON).
  - Результаты (заголовки, описания, SEO, категории).

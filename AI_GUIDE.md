# Документация AI-модуля Mudaroba
...
### Варианты товара: важное текущее правило

AI-обработка родительского товара не означает обязательную AI-обработку каждого варианта.

Текущая стратегия проекта:
- родительский AI использует варианты как контекст, чтобы не смешивать color/size differences в общее описание
- названия простых вариантов нормализуются дешёво, без LLM: `base model + color`
- отдельный `variant AI` запускается только когда у варианта есть собственные содержательные данные, а не просто другой цвет

Это сделано специально, чтобы не тратить токены на случаи, где различие варианта уже хорошо описывается структурой данных.
...
- **`ContentGenerator`**: Оркестратор. Запускает весь пайплайн обработки.
  ...
  - **Варианты**: собирает `variant_context` для родительского prompt и отдельно умеет делать cheap sync названий вариантов от базовой модели и цвета без LLM.
...
### 2.3 Общий словарь TR -> RU/EN

Для fashion-каталога AI и parser layers опираются на единый vocabulary:
...
- одинаково нормализовать `beyaz`, `lacivert`, `kirik beyaz`
- одинаково понимать `spor ayakkabi`, `çanta`, `gömlek`
- не дублировать ad-hoc словари в AI, serializers, parsers и category mapping

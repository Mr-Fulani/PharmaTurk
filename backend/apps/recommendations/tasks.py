...
def _exclude_variant_shadows(queryset):
    """Exclude legacy per-variant shadow products from the vector index."""
...
def remove_product_vectors(product_ids):
    """Remove non-public products from Qdrant, rechecking state at execution."""
    ...
    if not requested_ids:
        return {"removed": 0, "skipped_public": 0, "errors": []}
    ...
    for product_id in requested_ids:
        # A product may have been republished while this task waited in Celery.
        if product_id in public_ids:
            continue
        if engine.delete_product(product_id):
            removed += 1
        else:
            errors.append({"product_id": product_id, "error": "qdrant_delete_failed"})
    return {
        "removed": removed,
        "skipped_public": len(public_ids),
        "errors": errors,
    }
...
def index_product_vectors(product_ids=None, batch_size=100):
    ...
    if product_ids is not None:
        requested_ids = list(dict.fromkeys(int(product_id) for product_id in product_ids))
        queryset = _exclude_variant_shadows(
            public_recommendation_products().filter(id__in=requested_ids)
        )
        indexable_ids = set(queryset.values_list("id", flat=True))
        for product_id in requested_ids:
            if product_id not in indexable_ids:
                if engine.delete_product(product_id):
                    removed += 1
                else:
                    errors.append(
                        {"product_id": product_id, "error": "qdrant_delete_failed"}
                    )
        queryset = (
            queryset
        ...
        total_to_index = len(indexable_ids)
    else:
        base_qs = (
            _exclude_variant_shadows(public_recommendation_products())
            .filter(
...
def sync_all_products_to_qdrant():
    ...
    unpublished_ids = _unpublished_vector_product_ids()
    for offset in range(0, len(unpublished_ids), 100):
        remove_product_vectors.delay(product_ids=unpublished_ids[offset : offset + 100])
    return {"batches": batches, "deindex_submitted": len(unpublished_ids)}
...
def sync_stale_products_to_qdrant(batch_size=25, max_products=200):
    ...
    unpublished_ids = _unpublished_vector_product_ids(limit=max_products)
    if unpublished_ids:
        remove_product_vectors.apply_async(
            kwargs={"product_ids": unpublished_ids},
            priority=3,
        )
    ...
    return {
        "submitted": len(ids),
        "deindex_submitted": len(unpublished_ids),

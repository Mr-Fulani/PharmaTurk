...
class ScraperConfig(models.Model):
    ...
    priority = models.PositiveIntegerField(
        _("Приоритет"), default=100, help_text=_("Чем меньше число, тем выше приоритет")
    )

    # Настройки парсинга
    delay_min = models.FloatField(
        _("Минимальная задержка (сек)"),
        default=1.0,
        validators=[MinValueValidator(0.1), MaxValueValidator(60.0)],
    )
    delay_max = models.FloatField(
        _("Максимальная задержка (сек)"),
        default=3.0,
        validators=[MinValueValidator(0.1), MaxValueValidator(60.0)],
    )
    timeout = models.PositiveIntegerField(
        _("Таймаут запроса (сек)"),
        default=30,
        validators=[MinValueValidator(5), MaxValueValidator(300)],
    )
    max_retries = models.PositiveIntegerField(
        _("Максимум повторов"), default=3, validators=[MinValueValidator(1), MaxValueValidator(10)]
    )

    # Ограничения
    max_pages_per_run = models.PositiveIntegerField(
        _("Макс. страниц за запуск"),
        default=10,
        validators=[MinValueValidator(1), MaxValueValidator(1000)],
    )
    max_products_per_run = models.PositiveIntegerField(
        _("Макс. товаров за запуск"),
        default=100,
    ...
    sync_interval_hours = models.PositiveIntegerField(
        _("Интервал синхронизации (часы)"),
        default=24,
        validators=[MinValueValidator(1), MaxValueValidator(168)],
    )

    ai_on_create_enabled = models.BooleanField(
        _("AI для новых товаров"),
        default=True,
    )
    ai_on_update_enabled = models.BooleanField(
        _("AI для обновлений товаров"),
        default=True,
    )
    ...
    scraper_username = models.CharField(
        _("Логин парсера"),
        max_length=100,
        blank=True,
        help_text=_("Логин для авторизации парсера на сайте (если нужно)"),
    )
    scraper_password = models.CharField(
        _("Пароль парсера"),
        max_length=100,
        blank=True,
...
class CategoryMapping(models.Model):
    ...
    internal_category = models.ForeignKey(
        "catalog.Category", on_delete=models.CASCADE, verbose_name=_("Внутренняя категория")
    )
...
class BrandMapping(models.Model):
    ...
    internal_brand = models.ForeignKey(
        "catalog.Brand", on_delete=models.CASCADE, verbose_name=_("Внутренний бренд")
    )
...
class ScrapedProductLog(models.Model):
    ...
    product = models.ForeignKey(
        "catalog.Product", on_delete=models.SET_NULL, null=True, blank=True, verbose_name=_("Товар")
    )

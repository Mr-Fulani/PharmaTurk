...
class GlobalCurrencySettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Товарная маржа', {
            'fields': ('default_margin_percentage',),
            'description': (
                'Применяется только когда у бренда и категории товара указано 0%. '
                'Приоритет: бренд → категория → глобальная товарная маржа.'
            ),
        }),
        ('Валюты и криптовалюты', {
            'fields': ('usdt_markup_percentage',),
            'description': (
                'Маржа конвертации настраивается отдельно в «Маржа по валютным парам» и не участвует '
...
class ProductPriceAdmin(PublicProductPriceAdminMixin, admin.ModelAdmin):
    ...
    list_display = [
        ...
        'public_kzt_price', 'public_try_price', 'public_usdt_price',
...
class ProductVariantPriceAdmin(PublicProductPriceAdminMixin, admin.ModelAdmin):
    ...
    def _markup_product(self, obj):
        return getattr(getattr(obj, 'variant', None), 'product', None)

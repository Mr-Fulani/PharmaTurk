...
class TestNewAdminUploadProtection:
    ...
    def test_feedback_signals_do_not_download_remote_testimonial_media(self):
        """Remote testimonial URLs are validated in the API, never fetched in signals."""
        from apps.feedback import signals

        assert not hasattr(signals, "auto_download_testimonial_media")
        assert not hasattr(signals, "_download_and_save")

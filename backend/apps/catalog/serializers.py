...
from .models import (
    Category, CategoryTranslation, Brand, BrandTranslation, Product, ProductTranslation, ProductImage, PriceHistory,
...
from .pricing_serializers import (
    apply_markup_formatted as _pricing_apply_markup_formatted,
    apply_markup_value as _pricing_apply_markup_value,
    apply_product_markup_to_payload as _pricing_apply_product_markup_to_payload,
    effective_product_markup as _pricing_effective_product_markup,
    preferred_currency as _pricing_preferred_currency,
    public_price as _pricing_public_price,
)
...
def _preferred_currency(request, default="RUB"):
    return _pricing_preferred_currency(request, default)


def _public_price(amount, from_currency, request):
    return _pricing_public_price(
        amount,
        from_currency,
        request,
        currency_resolver=_preferred_currency,
    )


def _effective_product_markup(obj):
    return _pricing_effective_product_markup(obj)


def _apply_markup_value(value, margin):
    return _pricing_apply_markup_value(value, margin)


def _apply_markup_formatted(value, margin):
    return _pricing_apply_markup_formatted(
        value,
        margin,
        value_transform=_apply_markup_value,
    )


def _apply_product_markup_to_payload(data, obj):
    return _pricing_apply_product_markup_to_payload(
        data,
        obj,
        markup_resolver=_effective_product_markup,
        value_transform=_apply_markup_value,
        formatted_transform=_apply_markup_formatted,
    )
...
from .favorite_serializers import (
    AddToFavoriteSerializer as _AddToFavoriteSerializer,
    FavoriteSerializer as _FavoriteSerializer,
    resolve_product_for_favorites_api as _resolve_product_for_favorites_api,
)

AddToFavoriteSerializer = _AddToFavoriteSerializer
FavoriteSerializer = _FavoriteSerializer
resolve_product_for_favorites_api = _resolve_product_for_favorites_api
...
class ServicePortfolioItemSerializer(serializers.ModelSerializer):
    ...
    def get_result_summary(self, obj):
        lang = self._get_portfolio_lang()
        if lang == 'en' and obj.result_summary_en:
    ...
    def get_service_name(self, obj):
        ...
        if not service:
            return None
        lang = self._get_portfolio_lang()
        return _localized_related_name(service, lang) or None
...
class _SimpleDomainMixin(_LocalizedSeoMethodsMixin, serializers.Serializer):
    ...
    product_type = serializers.SerializerMethodField(read_only=True)
    _normalizes_public_prices = True

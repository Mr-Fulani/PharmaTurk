...
class GlobalCurrencySettings(models.Model):
    ...
    usdt_markup_percentage = models.DecimalField(
        ...
        help_text=(
            "Наценка магазина к паритету USDT/USD (1:1). Используется при расчёте любых курсов с USDT; "
            "ручные строки USDT в «Курс валют» для ценообразования не применяются. "
            "Дополнительная маржа — в настройках маржи по парам валют."
        ),
    )
...
class ServicePrice(models.Model):
    """Модель для хранения цен услуги с конвертацией во все валюты"""
    ...
    def __str__(self):
        return f"{self.service.name} - {self.base_price} {self.base_currency}"

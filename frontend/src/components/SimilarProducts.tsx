...
export default function SimilarProducts({
  ...
  const translateBadge = (reason: string): string => {
    ...
    if (trimmed === 'Рекомендуем' || trimmed === 'Recommended') return t('badge_recommend', 'Рекомендуем')
    if (trimmed === 'Очень похожий стиль' || trimmed === 'Very similar style') return t('badge_similar_style', 'Очень похожий стиль')
    if (trimmed === 'Похожий дизайн' || trimmed === 'Similar design') return t('badge_similar_design', 'Похожий дизайн')
    const brandMatch = trimmed.match(/^(?:Бренд|Brand)\s+(.+)$/i)
    if (brandMatch) return t('badge_brand', { brand: brandMatch[1], defaultValue: `Brand ${brandMatch[1]}` })
    const parts = trimmed.split(/\s*,\s*/).map((p) => {
      const p2 = p.trim()
      if (p2 === 'Рекомендуем' || p2 === 'Recommended') return t('badge_recommend', 'Рекомендуем')
      if (p2 === 'Очень похожий стиль' || p2 === 'Very similar style') return t('badge_similar_style', 'Очень похожий стиль')
      if (p2 === 'Похожий дизайн' || p2 === 'Similar design') return t('badge_similar_design', 'Похожий дизайн')
      const m = p2.match(/^(?:Бренд|Brand)\s+(.+)$/i)
      if (m) return t('badge_brand', { brand: m[1], defaultValue: `Brand ${m[1]}` })
      return p2
    })
    return parts.join(', ')
  }

...
## 9. Админка: дубли `ShoeProduct` без вариантов

Импорт иногда создаёт **отдельный родительский** `ShoeProduct` на каждый цвет вместо **`ShoeVariant`** у одного родителя. В списке «Товар обуви» по умолчанию применяется тот же смысл, что у публичного API: скрыты строки с `external_data.source_variant_*`, а также **пустые** родители (0 вариантов), если уже есть другой `ShoeProduct` с тем же `name`+`brand` и с хотя бы одним вариантом. В фильтре слева: **Объём списка → «Все записи»**, чтобы увидеть и удалить мусор вручную. Строки в БД не удаляются автоматически.

---

## 10. Что проверять после изменений в каталоге

...
### БАДы: availability-independent checkout — 2026-08-29

- [x] Зафиксирована новая бизнес-политика: БАД продаётся по последней актуализированной
  публичной цене независимо от наличия у Akakçe или другого supplier adapter.
- [x] Отсутствующий adapter, `out_of_stock`, неизвестный остаток и временная
  недоступность supplier source больше не блокируют корзину/checkout именно для
  `supplements`. Изменение цены продолжает использовать общий price acknowledgement.
- [x] Локальный legacy/synthetic `stock_quantity` БАДа не проверяется и не списывается
  при создании заказа; поэтому нулевой остаток не может отменить уже разрешённый checkout.
- [x] `OrderItem.supplier_confirmation_required=true` сохраняется и без source offer,
  чтобы администратор видел необходимость ручного исполнения заказа.
- [x] Старые `pending_confirmation` строки БАДов переводятся data migration в
  payable `not_checked` и безопасно перепроверяются при следующем cart mutation или
  checkout.
- [x] Видимый справочный блок проверки удалён из product detail. On-demand price check,
  polling, обновление основной цены, выбранная валюта и действующие наценки продолжают
  работать скрыто.
- [ ] Будущий богатый источник подключить через общий source-offer adapter для всех
  продаваемых категорий кроме медикаментов; Akakçe оставить одним из источников.

Этот раздел заменяет прежнее правило Akakçe negative canary, по которому

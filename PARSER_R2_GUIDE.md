...
## 5. Обработка данных с помощью AI (Новое)

После парсинга товары автоматически отправляются на обработку в AI-модуль.
Подробная документация по AI-системе доступна в отдельном файле:

👉 **[AI_GUIDE.md](AI_GUIDE.md)**

Модуль выполняет:
1. **Анализ изображений** (Vision API) для извлечения атрибутов.
2. **Автокатегоризацию** (RAG + LLM) для привязки к дереву категорий.
3. **Генерацию описаний** (SEO-оптимизированный текст на нужном языке).

### 5.1 Ручной запуск AI по сессии
Если нужно обработать конкретную сессию парсинга вручную:
1. Откройте **Admin Panel → Scrapers → Scraping sessions**.
2. Нажмите **«Запустить AI»** напротив нужной сессии.
3. Результаты смотрите в **AI module → Логи AI обработки**.

---

## 6. Чек-лист для нового парсера

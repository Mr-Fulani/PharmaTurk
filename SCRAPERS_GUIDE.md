...
- `SUPPORTS_PAGE_CHUNKING = True` ставить **только** если `parse_product_list`
  принимает `start_page` и реально начинает обход с этой страницы. Примеры:
  `IlacFiyatiParser` (пагинация `?pg=`), `LcwParser` (пагинация `?sayfa=`).
- Если парсер за один вызов забирает весь листинг (через API, без постраничного
  обхода — `ummaland`, `ikea`, `zara`), флаг **оставить `False`**. Иначе чепочка
...
**Akamai BotManager.** Zara закрыта Akamai. Перед первым AJAX-запросом сессия
«прогревается» одним GET на `https://www.zara.com/tr/tr/`, чтобы получить cookies
`ak_bmsc`/`_abck`/`bm_sz`; прогрев и AJAX используют один реалистичный User-Agent.
Защитная страница Akamai (HTTP 200 с маркером `bm-verify`) распознаётся и
поднимает ту же `ExternalAccessBlockedError`, что и явный 403 — админ видит
«Zara запретил доступ», а не пустую категорию.

**Жёсткая блокировка по IP.** Прогрев и заголовки не снимают репутационный бан
дата-центрового IP: Akamai стабильно отдаёт 403. Лечение — турецкий
residential/mobile прокси. Включается без правок кода: задать `SCRAPER_PROXY_URL`
в `.env` и поставить флаг **«Использовать прокси»** (`use_proxy`) в `ScraperConfig`.
Прокси применяется на уровне `BaseScraper` (httpx-клиент) и к requests-сессиям
...
**Группировка цветов.** Карточка модели объединяет все цвета: парсер берёт
`color_options`, дозагружает страницу каждого цвета и собирает один товар с
несколькими `fashion_variants` (цвет + его размеры), как Zara. Стабильный
`external_id = flo-<минимальный sku среди цветов>` — не зависит от того, с какого
цвета начали. В листинге другие цвета той же модели пропускаются (дедуп по sku),
чтобы не плодить дубли. Предохранитель — `MAX_COLOR_VARIANTS` цветов на карточку.
...
- `parse_product_list()` → листинг / категория / поиск. **Постранично** обходит
  категорию через `?sayfa=N` (page 1 — без параметра; `?page/pg/PageNumber` сайт
  игнорирует). Генератор: отдаёт товар сразу; стоп на пустой странице или когда новых
  товаров нет (LCW за последней страницей повторяет последнюю). Поддерживает `start_page`
  и `SUPPORTS_PAGE_CHUNKING=True` → авточепочка по чанкам.
...
> **Спеки в описании (важно для парфюма и не только).** Блок `Ürün İçeriği ve
> Özellikleri` LCW отдаёт парами `Метка:` / `Значение` на отдельных строках.
> `_build_rich_description` собирает их в описание по списку меток `direct_labels`
> (+ они же — границы в `next_labels_pattern`). Список включает и одёжные
> (`Ana Kumaş`, `Kumaş`, `Desen`…), и общие/перфюмные: `Cinsiyet`, `Paket İçeriği`,
> `Hacim`, `Koku Tipi`, `Form`, `Konsantrasyon`, `Net Miktar`. Описание держим
> «сырым» (одёжные секции не режем — их дочищает AI-агент). Из этого текста
> энричмент `_enrich_perfumery_attrs` программно проставляет поля
> `PerfumeryProduct`: `volume` (из `Hacim: 75 ml`), `fragrance_family` (из
> `Koku Tipi: Odunsu→woody`, `Fresh→fresh`, `Vanilya→gourmand`), `fragrance_type`
> (из EDP/EDT в названии/тексте). Добавляя новый тип товара с парсингом через LCW —
> убедись, что его метки есть в `direct_labels`, иначе они выпадут из описания.
...
#### Пауза и продолжение (большие каталоги)

Статусы задачи: `pending / running / paused / cancelled / completed / failed`.
Поле `resume_page` хранит страницу, с которой продолжить.

Кнопки в админке у задачи:
- **Пауза** (во время выполнения) — мягкая остановка: статус `paused` ловится
  опросом внутри чанка, чанк завершается штатно (товары сохранены),
  `resume_page` стоит на текущей странице. Счётчики **не** сбрасываются.
- **Продолжить (стр. N)** (у `paused`) — возобновляет с `resume_page`, передавая
  накопленные счётчики как базу (без сброса). Только для парсеров с пагинацией
  (`SUPPORTS_PAGE_CHUNKING`: ilacfiyati, lcw, zara, flo).
- **Перезапустить** — как и раньше, полный сброс счётчиков и `resume_page=1`,
  старт с 1-й страницы.
- **Остановить** — жёсткая остановка (`cancelled`), без возобновления.

`resume_page` пишется на старте каждого чанка (= его стартовая страница) и
сбрасывается в 1 при полном завершении — поэтому даже при падении воркера
«Продолжить» возьмёт начало недообработанного чанка. Дубли исключены
дедупликацией; цена повторного прохода чанка — повторная сетевая загрузка уже
собранных карточек (они идут как «пропущено»). Булк-действия: «Поставить на
паузу» / «Продолжить (с места остановки)». Для `stub_refresh` возобновление с
места пока не поддержано — используйте «Перезапустить».
...
## 17. Эксплуатационный контракт парсеров для AI-агента

Этот раздел — обязательная спецификация при диагностике и исправлении парсеров.
Нельзя считать задачу исправленной только потому, что парсер вернул товары один раз.

### 17.1. Два режима запуска обязаны быть эквивалентны

Каждый site-парсер должен корректно поддерживать:

1. URL категории/листинга — `parse_product_list()` находит карточки, а детали
   обрабатываются ограниченными Celery-чанками.
2. URL конкретного товара — ровно одна карточка проходит тот же путь нормализации,
   категоризации, бренда, медиа и сохранения.

Нельзя реализовывать отдельную урезанную модель данных для single-URL. При одинаковом
товаре оба режима должны сформировать совместимые `external_id`, `product_type`,
`attributes`, варианты, размеры и изображения.

### 17.2. Приоритет настроек задачи

Явно заданные значения в `SiteScraperTask`/`ScrapingSession` сильнее данных источника:

1. `target_subcategory` задачи;
2. `target_category` задачи;
3. mapping/default категории парсера.

Аналогично явно заданный `default_brand` задачи/конфига должен назначаться товару,
если это разрешено типом товара. Парсер не должен позднее затереть выбранные оператором
бренд или подкатегорию значением со страницы. Для книг бренд намеренно очищается.

### 17.3. Дедупликация между Celery-чанками

Идентичность карточки строится по `source + external_id`, а при отсутствии ID — по
каноническому URL без query string. Одна карточка не должна повторно попадать в соседние
чанки одной задачи. Проверять нужно не только дубликаты внутри списка, но и переходы
между чанками, паузу/продолжение и повторную доставку Celery-задачи.

`found` может отражать найденные ссылки, но суммы `created + updated + skipped` должны
соответствовать реально обработанным уникальным карточкам.

### 17.4. Мультиязычные URL и URL конкретной карточки

Нельзя определять тип страницы только по одному локализованному сегменту (`/urun/`,
`/product/`, `/kategori/`, `/category/`). Сначала использовать стабильный ID товара,
API/structured data/canonical URL; локализованные сегменты — только fallback.
Турецкие символы, ASCII-варианты slug и языковые префиксы `tr/en` должны приводиться
к одному каноническому маршруту, если источник обозначает ими один ресурс.

### 17.5. Идемпотентность обновления

Повторный запуск неизменившейся карточки обязан завершаться:

```text
found=1, created=0, updated=0, skipped=1
```

`updated` означает изменение бизнес-данных товара или состава связанных сущностей,
а не технический вызов `.save()`, обновление `updated_at`, повторную нормализацию файла
или удаление/создание тех же строк.

Запрещено без сравнения выполнять для вариантов и галерей:
...
Сначала сравнивается упорядоченный снимок полей. Строки пересоздаются только при
реальном изменении состава или порядка.

### 17.6. Медиа: `parsed` и читаемый путь — один логический файл

Обычный pipeline (кроме явно direct-readable парсеров):

```text
внешний URL
  -> products/parsed/<parser>/...       (скачивание и оптимизация)
  -> products/<category>/<product>/... (доменный readable upload_to)
```

Во время `pre_save` сигнал может скопировать файл в readable-путь и заменить
`image_url`. Поэтому входной parsed-URL и сохранённый readable-URL текстово различны,
хотя содержимое одинаково. Нельзя считать это изменением картинки по сравнению строк.
Для parser-owned внутренних файлов используется стабильная идентичность источника или
сравнение содержимого; одинаковые строки БД и конечные файлы должны сохранять прежние ID
и пути. Sweep `products/parsed` удаляет только файлы, на которые больше нет ссылок.

### 17.7. Обязательный порядок диагностики поломки

1. Зафиксировать `SiteScraperTask.id`, Celery task ID, исходный URL и время запуска.
2. Сопоставить счётчики задачи с логом конкретного task ID, а не со всем потоком worker.
3. Проверить HTTP-коды list/detail/API и количество элементов до/после нормализации.
4. Проверить `external_id`, канонический URL и dedup-key на границе чанков.
5. Проверить итоговые `target_category`, `target_subcategory`, `default_brand` в сессии.
6. Для ложного `updated` перечислить конкретные отличавшиеся поля. Если отличий нет —
   искать безусловные `.save()`, delete/recreate, timestamps и перенос медиа.
7. Проверить код внутри запущенного production-контейнера. `git pull` и
   `docker compose restart` не обновляют код в образе без bind mount.

### 17.8. Минимальные регрессионные тесты

Для исправленного или нового парсера обязательны:

- категория источника: несколько страниц/чанков без повторной обработки карточек;
- URL одного товара: создаётся одна карточка с теми же атрибутами;
- второй идентичный запуск: `updated=0`, `skipped=1`;
- изменение цены/остатка/варианта: `updated=1` и меняется только нужное поле;
- одинаковое parsed/readable изображение не пересоздаёт строку и файл;
- изменение состава/порядка изображений действительно обновляет галерею;
- заданные в задаче бренд и подкатегория имеют правильный приоритет;
- понятная ошибка сохраняется в `error_message`/`log_output`, а не превращается в
  успешный результат с нулём без объяснения.

После изменений общей интеграции прогонять не только тест конкретного источника, но и
регрессии всех потребителей общей ветки (например FLO, LCW, Zara/Inditex для fashion).

...
- Варианты: создавайте в инлайне `ShoeVariant`/`ClothingVariant` у родителя. Заполняйте цвет, размер, цену/валюту, остаток, `is_active`, при необходимости загрузите главное изображение и до 5 фото (флаг `is_main` обязателен, если нет `main_image` у варианта).
...
- Добавление в корзину:
  ...
  - Вариант или базовая карточка одежды/обуви: `product_type` (`clothing`/`shoes`) + `product_slug` (slug варианта, либо slug родителя, если вариантов нет).
- Фронт: на странице товара переключение цвета/размера выбирает slug варианта; AddToCart отправляет выбранный slug и `product_type`. Галерея показывает фото выбранного варианта, при его отсутствии — фото родителя.
